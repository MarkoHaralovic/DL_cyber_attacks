        ...
        if self.shape == (32, 32, 3):
            self.train_images = np.array(
                [np.array(resize_transform(Image.fromarray((img * 255).astype('uint8')))) for img in
                 tqdm(self.train_images, desc="Resizing train images")])
            self.test_images = np.array(
                [np.array(resize_transform(Image.fromarray((img * 255).astype('uint8')))) for img in
                 tqdm(self.test_images, desc="Resizing test images")])
        else:
            self.train_images = np.array(
                [np.array(resize_transform(Image.fromarray(img.transpose(1, 2, 0) * 255).astype('uint8'))) for img in
                 tqdm(self.train_images, desc="Resizing train images")]).transpose(0, 3, 1, 2)
            self.test_images = np.array(
                [np.array(resize_transform(Image.fromarray(img.transpose(1, 2, 0) * 255).astype('uint8'))) for img in
                 tqdm(self.test_images, desc="Resizing test images")]).transpose(0, 3, 1, 2)
